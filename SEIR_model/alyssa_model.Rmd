...
infection_plot_prop <- function(df, title, max_time) {
  if (is.null(max_time)) {
    max_time = max(times) + 1
  }
  df %>% filter(time < max_time) %>%
...
set_vacc <- function(disease, type) {
  ...
  if (type == "No Vaccination"){
    ...
    vaccS <- function(t){0}
      ...
      if (disease == "COVID-19") {
        vaccC <- vacc_weekly(0.642, WVH, k)
        vaccCA <- vacc_weekly(0.697, WVH, k)
        vaccP <- vacc_weekly(0.697, WVH, k)
        vaccS <- vacc_weekly(0.863, WVH, k)
      } else if (disease == "Flu") {
        vaccC <- vacc_weekly(0.597, WVH, k)
        vaccCA <- vacc_weekly(0.367, WVH, k)
        vaccP <- vacc_weekly(0.491, WVH, k)
        vaccS <- vacc_weekly(0.613, WVH, k)
      } else if (disease == "RSV") {
    ...
    if (disease == "COVID-19") {
        vaccC <- vacc_weekly(0.697, WVH, k)
        vaccCA <- vacc_weekly(0.697, WVH, k)
        vaccP <- vacc_weekly(0.697, WVH, k)
        vaccS <- vacc_weekly(0.863, WVH, k)
    } else if (disease == "Flu") {
        vaccC <- vacc_weekly(0.597, WVH, k)
        vaccCA <- vacc_weekly(0.597, WVH, k)
        vaccP <- vacc_weekly(0.597, WVH, k)
        vaccS <- vacc_weekly(0.613, WVH, k)
    } else if (disease == "RSV") {
        vaccC <- vacc_weekly(0.468, WVH, k)
...
```{R}
COVIDplotInfections60ct <- infection_plot(COVIDoutGrouped, "COVID-19 Infections Count (60 days)", 60)
ggsave("figures/COVIDplotInfections60ct.png", width = 10, height = 7, dpi = 300)
COVIDplotInfections60prop <- infection_plot_prop(COVIDoutGrouped, "COVID-19 Infections Proportion (60 days)", 60)
ggsave("figures/COVIDplotInfections60prop.png", width = 10, height = 7, dpi = 300)

COVIDplotInfections <- infection_plot(COVIDoutGrouped, "COVID-19 Infections Count (500 days)", NULL)
ggsave("figures/COVIDplotInfectionsct.png", width = 10, height = 7, dpi = 300)
COVIDplotInfectionsprop <- infection_plot(COVIDoutGrouped, "COVID-19 Infections Proportion (500 days)", NULL)
ggsave("figures/COVIDplotInfectionsprop.png", width = 10, height = 7, dpi = 300)
```
