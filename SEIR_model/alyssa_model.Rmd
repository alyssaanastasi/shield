...
outNoVac <- ode(y=init, func = seir, times=times, parms = parms) %>% 
  ...
  mutate(group = case_when(
      grepl("^S_C1|^S_C2|^E_C1|^E_C2|^I_C1|^I_C2|^E_C2|^R_C1|^R_C2|^D_C1|^D_C2|Inf_C1|^Inf_C2", name) ~ "Children",
      grepl("^S_CA|^E_CA|^I_CA|^R_CA|^D_CA|Inf_CA", name) ~ "Childless Adults",
      grepl("^S_P|^E_P|^I_P|^R_P|^D_P|Inf_P", name) ~ "Adults with Children (Parents)",
  )) %>%
    mutate(label = case_when(
      ...
      grepl("^Inf_.*1$", name) ~ "Infected (First)",
      grepl("^Inf_.*2$", name) ~ "Infected (Second)",
