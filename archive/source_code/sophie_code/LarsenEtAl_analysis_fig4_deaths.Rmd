...
outNoVac <- ode(y=init, func = seir, times=times, parms = parms) %>% 
  ...
  mutate(type = "No vaccination") %>% filter(str_detect(name, "D")) %>% 
  separate(name, c("Variable","SES"), sep = "_") %>%
  mutate(SES = str_replace_all(SES, "[:digit:]", "")) %>%
  group_by(time, type, SES) %>% summarise(CumDeath = sum(value)) %>%ungroup()
```
...
outC_4B <- ode(y=init, func = seir, times=times, parms = parms) %>% 
  as.data.frame() %>% as_tibble() %>% pivot_longer(-time) %>%
  mutate(type = "Concave") %>% filter(str_detect(name, "D")) %>% 
  separate(name, c("Variable","SES"), sep = "_") %>%
  mutate(SES = str_replace_all(SES, "[:digit:]", "")) %>%
  group_by(time, type, SES) %>% summarise(CumDeath = sum(value)) %>% 
  ungroup()
```
...
outS_4B <- ode(y=init, func = seir, times=times, parms = parms) %>% 
  as.data.frame() %>% as_tibble() %>% pivot_longer(-time) %>%
  mutate(type = "Sigmoidal") %>% filter(str_detect(name, "D")) %>% 
  separate(name, c("Variable","SES"), sep = "_") %>%
  mutate(SES = str_replace_all(SES, "[:digit:]", "")) %>%
  group_by(time, type, SES) %>% summarise(CumDeath = sum(value)) %>% 
  ungroup()
...
write_csv(out4B, "../../Data/data_figures/daily_vacc_t0_death.csv")

plotOut4B <- out4B %>% 
  ggplot(aes(x=time,y=CumDeath,col=type,linetype=SES)) + 
  geom_line()
```
...
outC_4C <- ode(y=initSec, func = seir, times=0:210, parms = parms) %>% 
  ...
  arrange(time) %>% filter(str_detect(name, "D")) %>% 
  separate(name, c("Variable","SES"), sep = "_") %>%
  mutate(SES = str_replace_all(SES, "[:digit:]", "")) %>%
  group_by(time, type, SES) %>% summarise(CumDeath = sum(value)) %>% 
  ungroup()
...
write_csv(out4C, "../../Data/data_figures/daily_vacc_t210_death.csv")
...
## Fig. 4D: Effect of vaccine on ections.
...
outC0_4D <- outNoVac %>% filter(time %in% c(210, max(time))) %>% 
  ...
  summarise(TotalDeath = sum(CumDeath)) %>% ungroup() %>%
  pivot_wider(names_from = time, 
              values_from = TotalDeath, names_prefix = "time_") %>%
  mutate(DeathsSinceMonth7 = round(time_420 - time_210, digits = 0)) %>%
  select(scenario,type,DeathsSinceMonth7)
```
...
outC1C_4D <- ode(y=initSec, func = seir, times=0:210, parms = parms) %>% 
  ...
  group_by(time,type,scenario) %>% summarise(TotalDeath = sum(value)) %>% 
  ungroup() %>% pivot_wider(names_from = time, values_from = TotalDeath, 
              names_prefix = "time_") %>%
  mutate(DeathsSinceMonth7 = round(time_420 - time_210, digits = 0)) %>%
  select(scenario,type,DeathsSinceMonth7)
...
out4D <- outC0_4D %>% rbind(outC1C_4D) %>% rbind(outC1S_4D) %>%
  ...
  mutate(Ref = outC0_4D$DeathsSinceMonth7) %>% 
  mutate(DeathsAdverted = round(100*(Ref - DeathsSinceMonth7)/Ref, digits = 0))

write_csv(out4D, "../../Data/data_figures/outcomes_4D_death.csv")
```
...
run_ode <- function() {
  x <- ode(y=initSec, func = seir, times=0:210, parms = parms) %>% 
    as.data.frame() %>% as_tibble() %>% pivot_longer(-time) %>%
    filter(time > 0) %>% mutate(time = time+max(outFirst$time)) %>%
    rbind(outFirst) %>% arrange(time) %>% filter(str_detect(name, "D")) %>% 
    separate(name, c("Variable","SES"), sep = "_") %>%
    mutate(SES = str_replace_all(SES, "[:digit:]", "")) %>% 
    filter(time %in% c(210, max(time))) %>%
    group_by(time) %>% summarise(TotalDeath = sum(value)) %>% 
    ungroup() %>% pivot_wider(names_from = time, values_from = TotalDeath, 
                names_prefix = "time_") %>%
    mutate(deltaDeaths = round(time_420 - time_210, digits = 0)) %>%
    mutate(ID = i) %>% select(ID, deltaDeaths)
  return(x)
}
...
out4E1 <- out4E1 %>% left_join(par4E1) %>% 
  mutate(type = "Cocave", ScenarioVm = "best case", 
         ScenarioWh = "best case", Ref = outC0_4D$DeathsSinceMonth7, 
         PercDeathAdverted = round(100*(Ref - deltaDeaths)/Ref, digits=0)) %>%
  select(type, ScenarioVm, ScenarioWh, Vm_H, Wh_H, PercDeathAdverted)
```
...
out4E2 <- out4E2 %>% left_join(par4E2) %>% 
  mutate(type = "Cocave", ScenarioVm = "best case", 
         ScenarioWh = "worst case", Ref = outC0_4D$DeathsSinceMonth7, 
         PercDeathAdverted = round(100*(Ref - deltaDeaths)/Ref, digits=0)) %>%
  select(type, ScenarioVm, ScenarioWh, Vm_H, Wh_H, PercDeathAdverted)
```
...
write_csv(out4E, "../../Data/data_figures/outcomes_4E_death.csv")
```
