...
dataCh <- dataCh %>%
  filter(measure == "meanIncome") %>%
  mutate(nbin = ntile(x= value, n = 4)) %>%
  mutate(cbin = cut_interval(value, n = 4, labels = c("L","ML","MH","H"), na.rm = T))

  dataCh$nbin[dataCh$nbin == 1] <- "L"
  dataCh$nbin[dataCh$nbin == 2] <- "ML"
  dataCh$nbin[dataCh$nbin == 3] <- "MH"
  dataCh$nbin[dataCh$nbin == 4] <- "H"
...
dataUSinc <- dataUS %>%
  ...
  dataUSinc$nbin[dataUSinc$nbin == 2] <- "ML"
  dataUSinc$nbin[dataUSinc$nbin == 3] <- "MH"
  dataUSinc$nbin[dataUSinc$nbin == 4] <- "H"
...
dataUSpov <- dataUS %>%
  ...
  group_by(state) %>%
  mutate(nbin = ntile(x= value, n = 4)) %>%
  mutate(cbin = cut_interval(value, n = 4, labels = c("H","MH","ML", "L"), na.rm = T)) %>%
  ungroup()

  dataUSpov$nbin[dataUSpov$nbin == 1] <- "H"
  dataUSpov$nbin[dataUSpov$nbin == 2] <- "MH"
  dataUSpov$nbin[dataUSpov$nbin == 3] <- "ML"
  dataUSpov$nbin[dataUSpov$nbin == 4] <- "L"
...
dataBe <- dataBe %>%
  ...
  dataBe$nbin[dataBe$nbin == 2] <- "ML"
  dataBe$nbin[dataBe$nbin == 3] <- "MH"
  dataBe$nbin[dataBe$nbin == 4] <- "H"
...
dataUKinc <- dataUK %>%
  ...
  dataUKinc$nbin[dataUKinc$nbin == 2] <- "ML"
  dataUKinc$nbin[dataUKinc$nbin == 3] <- "MH"
  dataUKinc$nbin[dataUKinc$nbin == 4] <- "H"
...
dataUKpov <- dataUK %>%
  ...
  dataUKpov$nbin[dataUKpov$nbin == 2] <- "MH"
  dataUKpov$nbin[dataUKpov$nbin == 3] <- "ML"
  dataUKpov$nbin[dataUKpov$nbin == 4] <- "L"
  ...
  dataUKpov2$nbin[dataUKpov2$nbin == 2] <- "MH"
  dataUKpov2$nbin[dataUKpov2$nbin == 3] <- "ML"
  dataUKpov2$nbin[dataUKpov2$nbin == 4] <- "L"
...
```{r}
diffmean <- dataGrid %>%
  filter(calctype == "H-L", measure == "meanIncome") 
...
```{r}
diffnettax <- dataGrid %>%
  filter(calctype == "H-L", measure == "income1") 
...
diffnettax %>% ggplot(aes(x = week, y = calc, group = country)) + geom_line()  + geom_hline(yintercept = 0, lty = 3) + facet_grid(
  rows = vars(bins),
  cols = vars(country)) + ggtitle("Difference H-L of avg vaccination (net taxable income)") + geom_vline(xintercept = 47)
```
...
```{r}
diffLSOA <- dataGrid %>%
  filter(calctype == "H-L", measure == "income3") 
...
diffLSOA %>% ggplot(aes(x = week, y = calc, group = country)) + geom_line()  + geom_hline(yintercept = 0, lty = 3) + facet_grid(
  rows = vars(bins),
  cols = vars(country)) + ggtitle("Difference H-L of avg vaccination (Prop. LSOAs in most deprived 10% nationally)") + geom_vline(xintercept = 50)
```
...
```{r}
diffMHI <- dataGrid %>%
  filter(calctype == "H-L", measure == "income5", state != "TX") %>%
  group_by(state) %>%
  filter(max(week, na.rm = T) > 10) %>%
  ungroup()
