...
datamake1 <- read_csv("makeCSV2.csv", col_types = cols (state = "c")) 
datamake2 <- read_csv("makeCSV3.csv", col_types = cols (state = "c"))
datamake3 <- read_csv("makeCSVPeru.csv", col_types = cols (state = "c")) %>%
  mutate(bins = NULL)
databr <- read_csv("Brazil.csv", col_types = cols (state = "c"))
...
d1 <- read_csv("loc2.csv", col_types = cols (state = "c")) %>%
  group_by(country, state) %>%
  filter(week == max(week)) %>%
  ungroup()

d2 <- read_csv("loc3.csv", col_types = cols (state = "c")) %>%
  group_by(country, state) %>%
  filter(week == max(week)) %>%
  ungroup()

d3 <- read_csv("locperu.csv") %>%
  add_column(state = NA, .after = 1) %>%
  group_by(country, state) %>%
  filter(week == max(week)) %>%
  ungroup()

d4 <- read_csv("locbr.csv", col_types = cols (state = "c")) %>%
