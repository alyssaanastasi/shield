...
dataCh2 <- read_csv("vaccination_Income_02-14-2022_Chile.csv")[c("Muncip","meanIncome")] %>%
...
dataUKinc <- dataUK %>%
  ...
  dataUKinc$nbin[dataUKinc$nbin == 5] <- "H"
...
dataUKpov <- dataUK %>%
  ...
  dataUKpov$nbin[dataUKpov$nbin == 5] <- "L"
  ...
  dataUKpov2$nbin[dataUKpov2$nbin == 5] <- "L"
...
plot <- makeCSV %>%
  filter(country != "United States") %>%
ggplot(aes(x = week, y = value)) + geom_line(aes(color = bin, group = bin), alpha=1, size = 1) + theme_minimal() + facet_wrap(~country)
plot
```
...
write.csv(makeCSV,"C:/Users/sophi/Box/Vaccine_inequity/Data/final/makeCSV3_5.csv", row.names = FALSE)

write.csv(dataMaster,"C:/Users/sophi/Box/Vaccine_inequity/Data/final/loc3_5.csv", row.names = FALSE)
