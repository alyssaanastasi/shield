...
#Clean data 
data$date <- as.Date(data$Day.of.Week.Ending, format = "%d-%b-%y")
data <- data[order(data$date), ]
data <- data[!duplicated(data), ]
```

# Get vaccination parameters from Survey
```{R}
# Age Bands -> 0-5, 6-18, 18-65, 65+
adult_vaccine_sample <- read.csv("vaccines_sample_adult.csv")

adult_design <- adult_vaccine_sample %>%
  as_survey_design(weights = weightvec)


adult_survey_filtered <- adult_design %>% 
  filter(self_age %in% c('19-29', '30-39', '40-49', '50-59'))

adults_results <- adult_survey_filtered %>% 
  summarize(
    prop_vax = survey_mean(covid_vax == 1, na.rm = TRUE, vartype = "ci")
  )

print(adults_results)

senior_filter <- adult_design %>% 
  filter(self_age %in% c('60-69', '70-79', '80+'))

senior_results <- senior_filter %>% 
  summarize(
    prop_vax = survey_mean(covid_vax == 1, na.rm = TRUE, vartype = "ci")
  )
...
child_vaccine_sample <- read.csv("vaccines_sample_child.csv")

child_design <- child_vaccine_sample %>%
  as_survey_design(weights = weightvec)

baby_filter <- child_design %>% 
  filter(child_age %in% c('0-5'))

baby_results <- baby_filter %>% 
  summarize(
    prop_vax = survey_mean(covid_vax == 1, na.rm = TRUE, vartype = "ci")
  )
...
adolescent_filter <- child_design %>% 
  filter(child_age %in% c('6-18'))

adolescent_results <- adolescent_filter %>% 
  summarize(
    prop_vax = survey_mean(covid_vax == 1, na.rm = TRUE, vartype = "ci")
  )

print(adolescent_results)
```

# COVID-19 Params
```{R}  
cov_data <- data %>% filter(Condition == "COVID-19") %>% mutate(day = (row_number()-1)*7) %>% rename(observed_hosps = Case.Count)
...
write.csv(dt_cov, "calibration_cov_04-2.csv", row.names = FALSE)
```
## Observe seasonality
...
# set seasonality
set_mat <- seas_matrix(cov_mu_estimates[1], cov_mu_estimates[2], cov_mu_estimates[3], 
                         cov_mu_estimates[4], cov_mu_estimates[5], cov_mu_estimates[6])
...
run_init["S_OC"] <- (1-pinf)*((1-pr)*(1-pexp)*run_init["S_OC"])
...
run_init["S_A"] <- (1-pinf)*((1-pr)*(1-pexp)*run_init["S_A"])
...
run_init["S_S"] <- (1-pinf)*((1-pr)*(1-pexp)*run_init["S_S"])
...
test_out <- run_ode(run_init, seir_COV, 0:3000  , currvac_parms)
...
```{R}
# Check Sums 
test_sums <- test_out %>% group_by(time) %>% summarise(total_at_time = sum(value))
```
...
plot_df <- test_out %>% filter(type != "Susceptible") %>% group_by(type, time) %>% summarise(overall = sum(value))

ggplot(plot_df, aes(x = time, y = overall, color=type)) +
    geom_line(alpha = 0.8) +
    labs(
      title = "Test Output",
      x = "Day",
