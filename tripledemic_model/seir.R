...
seir <- function(t, y, pars){
  ...
  # Get current total hospitalizations
  H_now <- get_H_total(y)
  
  # At this time step, how many people would be added? 
  dH_future <- probHC_RSV_vax*gammaI_RSV*I_C_RSV_vax + probHC_RSV*gammaI_RSV*I_C_RSV + 
              probHC_COV_vax*gammaI_COV*I_C_COV_vax + probHC_COV*gammaI_COV*I_C_COV + 
              probHC_FLU_vax*gammaI_FLU*I_C_FLU_vax + probHC_FLU*gammaI_FLU*I_C_FLU
  
  if (dH_future > 0){
    fractional_term <- (H_cap - H_now) / dH_future
  ...
  # Updates
  dS_C <- -((lambdaC_RSV * ((vaccC_RSV * ve_RSV) +  (1-vaccC_RSV))) +  
            (lambdaC_COV * ((vaccC_COV * ve_COV) +  (1-vaccC_COV))) + 
            (lambdaC_FLU * ((vaccC_FLU * ve_FLU) +  (1-vaccC_FLU))))*S_C + 
            omega_RSV*R_C_RSV + omega_COV*R_C_COV + omega_FLU*R_C_FLU
  dS_OC <- -((lambdaOC_RSV * ((vaccOC_RSV * ve_RSV) +  (1-vaccOC_RSV))) +  
               (lambdaOC_COV * ((vaccOC_COV * ve_COV) +  (1-vaccOC_COV))) + 
               (lambdaOC_FLU * ((vaccOC_FLU * ve_FLU) +  (1-vaccOC_FLU))))*S_OC + 
              omega_RSV*R_OC_RSV + omega_COV*R_OC_COV + omega_FLU*R_OC_FLU
  dS_A <- -((lambdaA_RSV * ((vaccA_RSV * ve_RSV) +  (1-vaccA_RSV))) +  
              (lambdaA_COV * ((vaccA_COV * ve_COV) +  (1-vaccA_COV))) + 
              (lambdaA_FLU * ((vaccA_FLU * ve_FLU) +  (1-vaccA_FLU))))*S_A + 
              omega_RSV*R_A_RSV + omega_COV*R_A_COV + omega_FLU*R_A_FLU
  dS_S <- -((lambdaS_RSV * ((vaccS_RSV * ve_RSV) +  (1-vaccS_RSV))) +  
              (lambdaS_COV * ((vaccS_COV * ve_COV) +  (1-vaccS_COV))) + 
              (lambdaS_FLU * ((vaccS_FLU * ve_FLU) +  (1-vaccS_FLU))))*S_S + 
            omega_RSV*R_S_RSV + omega_COV*R_S_COV + omega_FLU*R_S_FLU
...
dD_S_COV <- alphaS_COV*gammaH_COV*H_S_COV + (1-prop_can_be_hospitalized)*probHS_COV*gammaI_COV*I_S_COV
  ...
  diff_sum <- sum(c(dS_C, dS_OC, dS_A, dS_S,
  ...
  # print(paste0("Sum of diff eqs ", diff_sum))
  
  # compute total for Children (living + D if you want) -> Helps for tracking & debugging
dS  <- dS_C
# RSV group (include vax and unvax, H, I, R, D)
dRSV <- dE_C_RSV_vax + dI_C_RSV_vax + dH_C_RSV_vax  +
        dE_C_RSV + dI_C_RSV + dH_C_RSV +
        dR_C_RSV
# COV group
dCOV <- dE_C_COV_vax + dI_C_COV_vax + dH_C_COV_vax  +
        dE_C_COV + dI_C_COV + dH_C_COV  +
        dR_C_COV
# FLU group
dFLU <- dE_C_FLU_vax + dI_C_FLU_vax + dH_C_FLU_vax  +
        dE_C_FLU + dI_C_FLU + dH_C_FLU  +
        dR_C_FLU

dN_children <- dS + dRSV + dCOV + dFLU

# print or save
# cat("dN_children =", dN_children, "\n")
# cat(" breakdown: dS=", dS, " dRSV=", dRSV, " dCOV=", dCOV, " dFLU=", dFLU, "\n")
