...
#' Removes R compartment to help with endemicity 
...
seir2 <- function(t, y, pars){
  ...
  list2env(as.list(get_lambdas_without_R(t, pars, y, cmat)), envir=environment()) #get_lambdas is function used to compute lambda for each age & disease
  ...
  # Updates
  dS_C <- -(lambdaC_RSV + lambdaC_COV + lambdaC_FLU)*S_C +
  ...
  dS_OC <- -(lambdaOC_RSV + lambdaOC_COV + lambdaOC_FLU)*S_OC + 
  ...
  dS_A <- -(lambdaA_RSV + lambdaA_COV + lambdaA_FLU)*S_A + 
  ...
  dS_S <- -(lambdaS_RSV + lambdaS_COV + lambdaS_FLU)*S_S + 
