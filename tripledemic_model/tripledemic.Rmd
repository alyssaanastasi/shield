...
## No Vaccination
```{R}
df_novac <- run_ode(init=init, seir_fn=seir_rev, times=times, parms=novac_parms)
```

```{R}
novac_plot_ct <- plot_infections_count_by_vacc(df_novac, 400, novac=TRUE)
...
## Current Vaccination
```{R}
df_currvac <- run_ode(init=init, seir_fn=seir_rev, times=times, parms=currvac_parms)
```
...
currvac_deaths <- plot_cum_deaths(df_currvac, novac=TRUE)
